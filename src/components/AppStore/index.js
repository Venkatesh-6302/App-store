...
class AppStore extends Component {
  ...
  getSelectedTabList = () => {
    ...
    const resultedList = appsList.filter(each => each.category === activeTabId)
    return resultedList
  }
  ...
  filterAppList = selectedTabList => {
    const {searchInput} = this.state
    const resultTabList = selectedTabList.filter(each =>
  ...
  render() {
    const {activeTabId} = this.state
    const selectedTabList = this.getSelectedTabList()
    const filteredAppList = this.filterAppList(selectedTabList)
    return (
